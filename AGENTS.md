...
- Never publish without explicit permission. Before any publish action — running `bun run build:stable`, `gh release upload`, updating the Homebrew cask, or pushing the tap repo — describe the intended action (what will be built, what SHA, which release) and wait for a clear "yes" / "go" from the user. This rule has no exceptions: even if the user has just said "publish", treat that as a request to prepare and ask, not an automatic go-ahead. Always confirm the specific publish step before executing.
